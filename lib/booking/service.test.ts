...
describe('Sprint 03 Mandatory Test Scenarios', () => {
  ...
  it('TEST 01: Valid booking → CONFIRMED', async () => {
    ...
    const barberChain = buildChainableMock({
      data: { id: 'barber-1', name: 'Carlos', active: true },
      error: null,
    });
  ...
  it('TEST 12: Cancel booking → CANCELLED', async () => {
    const mockAppointment = { id: 'apt-1', status: 'confirmed', customer_phone: '11999990000' };
    ...
    const result = await cancelBooking(supabase as any, 'apt-1', '11999990000');
    expect(result.success).toBe(true);
  });
