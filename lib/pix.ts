...
export function generatePixCopiaECola(params: PixPayloadParams): string {
  const { pixKey, beneficiaryName, city, amount, txId = 'BARBEROS' } = params;
  ...
  const cleanName = beneficiaryName
    ...
    .substring(0, 25)
    .toUpperCase();
  const nameField = emvField('59', cleanName || SALON.pix.beneficiario.substring(0, 25).toUpperCase());
  ...
  const cleanCity = city
    ...
    .substring(0, 15)
    .toUpperCase();
  const cityField = emvField('60', cleanCity || SALON.pix.cidade.substring(0, 15).toUpperCase());

  // Additional Data Field (Tag 62) -> txId (Tag 05)
  const cleanTxId = txId.replace(/[^A-Za-z0-9]/g, '').substring(0, 25) || 'BARBEROS';
