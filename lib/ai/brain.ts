...
/**
 * CÉREBRO DO ASSISTENTE DIGITAL — SPRINT 4
 * 
 * Arquitetura:
...
export async function pensarEResponder(
  ...
  if (perguntouServicoInexistente) {
    ...
    return {
      ...
      quickReplies: [
        { label: 'Falar com Proprietário', action: 'FALAR_COM_HUMANO' },
  ...
  if (
    ...
    if (preco.success && preco.data) {
      ...
      const upsell = sugerirUpsell(preco.data, SALON_KNOWLEDGE_BASE.servicos);
      const crossSell = sugerirCrossSell(preco.data, SALON_KNOWLEDGE_BASE.servicos);
  ...
  if (norm.includes('corte social') || norm.includes('social')) {
    const srv = SALON_KNOWLEDGE_BASE.servicos.find(s => s.nome.toLowerCase().includes('social'))!;
    return {
      reply: `Perfeito! *${srv.nome}* — R$ ${srv.preco} (${srv.duracaoMinutos} min). 😊\n\nCom quem você prefere ser atendido?`,
      intent: 'SELECT_SERVICE',
      component: 'barbers_list',
      quickReplies: [
        { label: 'barber-1', action: 'SELECT_BARBER', payload: { barberName: 'barber-1' } },
        { label: 'Carlos', action: 'SELECT_BARBER', payload: { barberName: 'Carlos' } },
  ...
  if (norm.includes('degrade') || norm.includes('degradê')) {
    const srv = SALON_KNOWLEDGE_BASE.servicos.find(s => s.nome.toLowerCase().includes('degradê'))!;
    return {
  ...
  if (norm.includes('barba')) {
    const srv = SALON_KNOWLEDGE_BASE.servicos.find(s => s.nome.toLowerCase().includes('barba'))!;
    return {
  ...
  if (norm.includes('cabelo e barba') || norm.includes('cabelo + barba') || norm.includes('combo')) {
    const srv = SALON_KNOWLEDGE_BASE.servicos.find(s => s.nome.includes('+'))!;
    return {
  ...
  if (norm.includes('meia hora') || norm.includes('30 min') || norm.includes('pouco tempo') || norm.includes('algo rapido') || norm.includes('apressado')) {
    const servicosRapidos = SALON_KNOWLEDGE_BASE.servicos.filter(s => s.duracaoMinutos <= 30);
    return {
  ...
  if (
    ...
    return {
      ...
      quickReplies: [
        { label: 'Menu inicial', action: 'GREETING' },
        { label: 'Falar com Proprietário', action: 'HUMAN_HANDOFF' }
      ]
    };
  }
