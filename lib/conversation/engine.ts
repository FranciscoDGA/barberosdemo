    ...
    if (!userApt) {
      return {
        reply: {
          ...
          timestamp: timeStr,
          quickReplies: [
            { label: `Falar com ${SALON.nome}`, action: 'FALAR_COM_HUMANO' }
          ]
        },
        nextStep: 'IDLE',
        nextDraft: draft
...
function handleStructuredAction(
  ...
  if (action === 'ESCOLHER_SERVICO') {
    ...
    return {
      reply: {
        ...
        payload: { barbers },
        quickReplies: [
          { label: 'barber-1', action: 'ESCOLHER_BARBEIRO', payload: { barberName: 'barber-1' } },
          { label: 'Carlos', action: 'ESCOLHER_BARBEIRO', payload: { barberName: 'Carlos' } },
  ...
  if (action === 'ESCOLHER_BARBEIRO') {
    const bName = data?.barberName || 'barber-1';
    ...
    return {
      ...
      actionToExecute: {
        ...
        payload: {
          ...
          customerPhone: draft.customerPhone || SALON.whatsapp,
