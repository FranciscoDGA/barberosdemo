...
export async function processDueReminders(
  ...
  try {
    ...
    for (const item of reminders) {
      ...
      let reminderTitle = `🚨 ${SALON.lembretes.titulo3BIBE}: Horário em 15 minutos! (Tolerância 10 min)`;

      if (item.reminder_type === 'reminder_1') {
        bibeType = 'lembrete_24h';
        reminderTitle = `🔔 ${SALON.lembretes.titulo1BIBE} — ${SALON.nome}`;
      } else if (item.reminder_type === 'reminder_2') {
        bibeType = 'lembrete_2h';
        reminderTitle = `⏰ ${SALON.lembretes.titulo2BIBE} — ${SALON.nome}`;
      }
  ...
  try {
    ...
    switch (type) {
      ...
      case 'BOOKING_EXPIRED': {
        ...
        const clientMsg =
          `O período de tolerância de ${SALON.regras.toleranciaMinutos} minutos foi ultrapassado.\n\n` +
        ...
        const storeMsg =
          ...
          `Cliente: ${customerName} não compareceu dentro da tolerância de ${SALON.regras.toleranciaMinutos} minutos.\n` +
