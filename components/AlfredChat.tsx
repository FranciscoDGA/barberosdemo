...
export default function AlfredChat({ className = '' }: AlfredChatProps) {
  ...
  useEffect(() => {
    if (isOpen && !hasGreeted) {
      setMessages([{
        role: 'assistant',
        content: `Olá! Sou o Alfred, assistente da ${SALON.nome}. Como posso ajudar? Posso tirar dúvidas sobre serviços, preços, horários e disponibilidade.`
      }]);
      setHasGreeted(true);
    }
  }, [isOpen, hasGreeted]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: `Desculpe, tive um problema técnico. Tente novamente ou acesse ${SALON.url}/agendar para agendar diretamente.`
      }]);
    } finally {
      setIsLoading(false);
    }
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-emerald-500 to-emerald-600">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-black/60 font-medium">Assistente {SALON.nome}</p>
              </div>
            </div>
            <button
