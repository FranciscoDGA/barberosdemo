...
export const MyAppointments: React.FC<{ onOpenReviewModal?: (apt: Appointment) => void }> = ({
  ...
  const handleCancelAppointment = (id: string) => {
    if (confirm(`Deseja realmente cancelar este agendamento na ${SALON.nome}?`)) {
      updateAppointmentStatus(id, 'cancelled');
    }
  };

  const createGoogleCalendarUrl = (apt: Appointment) => {
    const title = encodeURIComponent(`${SALON.nome}: ${apt.serviceNames.join(', ')}`);
    const details = encodeURIComponent(
      `Agendamento com ${apt.barberName} no ${SALON.nome}. Valor: R$ ${apt.totalPrice.toFixed(2)}`
    );
    const location = encodeURIComponent(`${SALON.nome} - ${SALON.endereco.rua}`);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Acompanhe seus horários marcados e histórico na {SALON.nome}.
          </p>
        </div>
