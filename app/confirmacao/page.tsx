...
function ConfirmacaoContent() {
  ...
  const whatsappMessage = encodeURIComponent(
    `Fala!\n\nAgendei horario na *${SALON.nome}*:\n\n*${service}*\n${barber}\n${formattedDate} as ${time}\nR$ ${price}\n${paymentLabels[payment] || payment.toUpperCase()}\n\nAté lá!`
  );

  const googleMapsUrl = SALON.endereco.googleMaps;
      ...
      <header className="sticky top-0 z-40 w-full bg-[#090d16]/95 backdrop-blur-lg border-b border-slate-800/80">
        <div className="max-w-lg mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-black text-sm tracking-wider text-white">{SALON.nome.toUpperCase()}</span>
          </div>
          <PWAInstallButton />
        </div>
      </header>
        ...
        <div className="space-y-6 animate-in fade-in">
          ...
          <div className="flex flex-col items-center text-center space-y-3">
            ...
            <p className="text-sm text-slate-400">
              Seu horário na <strong className="text-amber-400">{SALON.nome}</strong> está confirmado.
            </p>
          </div>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-4 space-y-3">
            ...
            <p className="text-xs text-slate-300">{SALON.endereco.cidade} - {SALON.endereco.estado}, CEP {SALON.endereco.cep}</p>
            <a
              href={googleMapsUrl}
              target="_blank"
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-4 space-y-3">
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="flex gap-2 w-full">
                <a
                  href={`https://wa.me/?text=${encodeURIComponent(`Fala! Agende seu horário na ${SALON.nome}! 💈\n\n${bookingUrl}\n\nAté lá!`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
          ...
          <div className="space-y-3">
            <a
              href={`https://wa.me/${SALON.whatsapp}?text=${whatsappMessage}`}
              target="_blank"
              rel="noopener noreferrer"
