...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      services = SALON_KNOWLEDGE_BASE.servicos.map(s => ({
        id: s.id,
        name: s.nome,
        category: 'cabelo',
    ...
    try {
      ...
      barbers = SALON_KNOWLEDGE_BASE.barbeiros.map(b => ({
        id: b.id,
        name: b.nome,
        role: 'barbeiro',
        avatarUrl: '',
    ...
    try {
      ...
      brainOutput = {
        reply: 'Desculpe, tive uma dificuldade momentânea. Por favor, tente novamente ou fale diretamente com o proprietário no WhatsApp.',
        intent: 'AI_FALLBACK',
        quickReplies: [
...
export async function GET() {
  return NextResponse.json({
    status: 'ok',
    service: `${SALON.nome} Digital Employee - WhatsApp Webhook`,
